...
-- bash -c "cd ~/PX4-Autopilot && PX4_GZ_MODEL_POSE=\"5.5,10.0,0,0,0,0.0\" PX4_GZ_WORLD=$MAP_NAME make px4_sitl gz_rover_ackermann; exec bash"
...
-- bash -c "cd ~/rover_ackermann && source install/setup.bash && ros2 launch ackermann_rover_bringup pure_navigation.launch.py map_name:=$MAP_NAME initial_pose_x:=5.5 initial_pose_y:=10.0 initial_pose_yaw:=0.0; exec bash"
